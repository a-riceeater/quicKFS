...
The macOS adapter owns the synchronous-callback/async-runtime bridge. One multi-thread Tokio runtime creates the authenticated `NetworkFilesystem` and remains shared by all macFUSE callbacks for the life of the mount. Stable local inode and handle tables translate FUSE integers to opaque protocol node IDs and session-scoped remote handles. Windows support is expected to use WinFsp later.
