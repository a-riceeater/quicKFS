//! Native FFI bindings to libfuse2.
//!
//! This is a small set of bindings that are required to mount/unmount FUSE filesystems and
//! open/close a fd to the FUSE kernel driver.

#![warn(missing_debug_implementations)]
#![allow(missing_docs)]

use libc::c_char;
use libc::c_int;

#[repr(C)]
#[derive(Debug)]
pub(crate) struct fuse_args {
    pub argc: c_int,
    pub argv: *const *const c_char,
    pub allocated: c_int,
}

#[cfg(all(fuser_mount_impl = "libfuse2", target_os = "macos"))]
#[repr(C)]
pub(crate) struct fuse_chan {
    _private: [u8; 0],
}

#[cfg(fuser_mount_impl = "libfuse2")]
unsafe extern "C" {
    // *_compat25 functions were introduced in FUSE 2.6 when function signatures changed.
    // Therefore, the minimum version requirement for *_compat25 functions is libfuse-2.6.0.

    #[cfg(not(target_os = "macos"))]
    pub(crate) fn fuse_mount_compat25(mountpoint: *const c_char, args: *const fuse_args) -> c_int;

    // macFUSE 5.3 no longer implements the API-25 compatibility mount path.
    // Use its current channel API and duplicate the channel descriptor for
    // fuser's independently owned DevFuse handle.
    #[cfg(target_os = "macos")]
    pub(crate) fn fuse_mount(
        mountpoint: *const c_char,
        args: *mut fuse_args,
    ) -> *mut fuse_chan;
    #[cfg(target_os = "macos")]
    pub(crate) fn fuse_chan_fd(channel: *mut fuse_chan) -> c_int;
    #[cfg(target_os = "macos")]
    pub(crate) fn fuse_unmount(mountpoint: *const c_char, channel: *mut fuse_chan);
    #[cfg(not(any(
        target_os = "macos",
        target_os = "freebsd",
        target_os = "dragonfly",
        target_os = "openbsd",
        target_os = "netbsd"
    )))]
    pub(crate) fn fuse_unmount_compat22(mountpoint: *const c_char);
}
