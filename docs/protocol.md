...
**Both tiers self-heal.** Neither ceiling is a hard wall that fails the request that hits it. When the *global* pool is full, referencing a new node first evicts a globally-unreferenced cached entry and retries. The *per-connection* working set behaves the same way: when its permits are exhausted it evicts one of its own nodes — skipping the root and any node currently backing an open handle — and retries, so the working set is a bounded cache over the larger global registry exactly as the global registry is a bounded cache over the export. This matters because macOS reclaims vnodes lazily: a long-lived mount can reference more distinct nodes over its lifetime than the cap without ever sending a `Forget`, and without this a mount that had been up long enough would fail *every* `getattr`/`lookup` of a new node with *"too many known nodes"* until it was remounted. Evicting a per-connection node only drops that connection's reference; the node stays in the global registry (four times larger by default), so the kernel's next access re-tracks it from cache rather than paying a rediscovery walk, and its stable identifier is unchanged so nothing the client cached goes stale. A request can still surface *"too many known nodes"* only in the pathological case where every permit is pinned by a live open handle (bounded by `--max-open-handles`, `1024` by default — far below the node cap), which no real workload reaches.

**Why the default is 131072 / 524288.** macOS keeps a large kernel vnode cache (`kern.maxvnodes` is typically 100k–300k), so a Finder browse, Spotlight index, or recursive `find` over a large media library legitimately holds tens of thousands of live inodes on a single mount. The macFUSE mount also runs with `auto_xattr` (extended attributes stored in AppleDouble `._name` sidecars, required so `cp`/Finder can copy quarantined downloads — see [filesystem-semantics.md](filesystem-semantics.md)), which makes the kernel look up a `._name` node per file and roughly **doubles** the working set during a crawl. The historical `8192 / 65536` defaults were sized for small exports and undersized for these workloads. With self-healing eviction (above) an undersized per-connection cap no longer fails the request — it degrades to eviction *churn*, re-tracking cached nodes as the working set rotates through the cap — but a cap comfortably above the kernel's live vnode count avoids that churn entirely. (Historically, before per-connection eviction, exhaustion instead surfaced as `opendir`/`readdir`/`getattr` failing with *"too many known nodes"*, delivered as `ErrorCode::TooLarge` and mapped by the mount to `EFBIG` / "File too large" — the failure this self-heals.)
